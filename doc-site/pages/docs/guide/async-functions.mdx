...
const { result, isLoading, error } = useAsyncLookup({
    trigger: 'SHALLOW',
    execute: getUsers,
    query: { keywords },
});
...
The third highlighted row shows how you pass it to `useAsyncLookup`.
...
the pagination state which will trigger a new call to `useAsyncLookup`.

[useAsyncLookup](doc:useAsyncLookup) also provides the `accumulatePages` option
...
[useAsyncValue](doc:useAsyncValue) is useful
