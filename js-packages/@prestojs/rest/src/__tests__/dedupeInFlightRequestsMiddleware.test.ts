...
test('should de-dupe in flight requests', async () => {
    ...
    const endpoint1 = new Endpoint(new UrlPattern('/whatever/'), {
        middleware: [dedupeInFlightRequestsMiddleware()],
    });
    const p1 = endpoint1.execute();
    const p2 = endpoint1.execute();
    ...
    expect((await endpoint1.execute()).result).toBe('hello world 1');
    expect(fetchMock).toHaveBeenCalledTimes(2);
    const init1 = { headers: new Headers({ 'x-test': '123' }) };
    const p3 = endpoint1.execute(init1);
    const p4 = endpoint1.execute(init1);
    ...
    expect((await endpoint1.execute(init1)).result).toBe('hello world 3');
    expect(fetchMock).toHaveBeenCalledTimes(4);
    const init5 = { headers: new Headers({ 'x-test': 'abc' }) };
    const init6 = { headers: new Headers({ 'x-test': '123' }) };
    const p5 = endpoint1.execute(init5);
    const p6 = endpoint1.execute(init6);
    ...
    const p7 = endpoint1.execute({ headers: new Headers({ 'x-test1': '1', 'x-test2': '2' }) });
    const p8 = endpoint1.execute({ headers: new Headers({ 'x-test2': '2', 'x-test1': '1' }) });
...
test('should de-dupe in flight requests including query parameters in any order', async () => {
    ...
    const p1 = endpoint1.execute({ query: { param1: '1', param2: '2' } });
    const p2 = endpoint1.execute({ query: { param2: '2', param1: '1' } });
    expect((await p1).result).toBe('hello world 0');
    expect((await p2).result).toBe('hello world 0');
    expect(fetchMock).toHaveBeenCalledTimes(1);
    const p3 = endpoint1.execute({ query: { param1: '1', param2: '2' } });
    const p4 = endpoint1.execute({ query: { param2: 'two', param1: '1' } });
    expect((await p3).result).toBe('hello world 1');
    expect((await p4).result).toBe('hello world 2');
    expect(fetchMock).toHaveBeenCalledTimes(3);
    const p5 = endpoint1.execute({ query: { param1: ['one', 'two'], param2: 'test?this?' } });
    const p6 = endpoint1.execute({ query: { param1: ['one', 'two'], param2: 'test?this?' } });
    const p7 = endpoint1.execute({ query: { param1: 'one', param2: 'test' } });
...
test('should support customising key', async () => {
    ...
    const p1 = endpoint1.execute();
    const p2 = endpoint1.execute();
    const p3 = endpoint1.execute({ headers: { 'x-test': '123' } });
    expect((await p1).result).toBe('hello world 0');
    expect((await p2).result).toBe('hello world 0');
    ...
    expect((await endpoint1.execute()).result).toBe('hello world 1');
    expect(fetchMock).toHaveBeenCalledTimes(2);
    const p4 = endpoint1.execute();
    const p5 = endpoint1.execute({ method: 'post' });
    expect((await p4).result).toBe('hello world 2');
    expect((await p5).result).toBe('hello world 3');
    expect(fetchMock).toHaveBeenCalledTimes(4);
});
...
test('should support customising test', async () => {
    ...
    const p1 = endpoint1.execute();
    const p2 = endpoint1.execute({ method: 'POST' });
    const p3 = endpoint1.execute({ method: 'PUT' });
    expect((await p1).result).toBe('hello world 0');
    expect((await p2).result).toBe('hello world 0');
    expect((await p3).result).toBe('hello world 0');
    expect(fetchMock).toHaveBeenCalledTimes(1);
